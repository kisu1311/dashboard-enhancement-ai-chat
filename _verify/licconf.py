...
def tokens_source(src_text):
    ...
    SKIPPED the most-edited file while still printing a pass. On 12 Sep 2026 that stylesheet was
    folded into `setting.js`, where it lives inside a template literal, so the fallback reads
    THAT file and unescapes the backticks the literal requires. Both shapes still have to work:
    a page may carry the block inline (none do today) or leave it to the module."""
    if re.search(r'\n#agPage[^{]*\{', src_text):
        return src_text
    ext = ROOT / 'setting.js'
    if ext.exists():
        # ⚠️ UNESCAPE. `setting.js` holds the stylesheet in a template literal, so every
        #    backtick in it is written \` . The CSS parser here never sees the file, but a
        #    stray backslash inside a comment would be carried into the adapter <style>.
        return ext.read_text(encoding='utf-8').replace('\\`', '`')
    return src_text
